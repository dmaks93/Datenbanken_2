...
/**
 * DirectNativeQueries. Sample code for the lecture Databases 2.
 *
 * @author Martin Abel
...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      try {
        // Customized Constructor SqlResultSetMapping: Double
        Query query1 = em.createNativeQuery(
            "select max(e.esalary) as maxSal from Employee e",
            "Employee.ResMap.MaximumSalary");
      ...
      Query query2 = em.createNativeQuery("select UPPER(e.ename) from Employee e");
      ...
      Query query3 = em.createNativeQuery(
          "select e.ename, e.edeg from Employee e "
              + "where e.ename like '%t%' and e.esalary between 40000 and 50000 "
              + "order by e.esalary desc", "Employee.ResMap.Entity");
      List<Employee> result3 = (List<Employee>) query3.getResultList();
      for (Employee e : result3) {
        System.out.println(e.getEname() + "\t" + e.getEdeg());
      }
